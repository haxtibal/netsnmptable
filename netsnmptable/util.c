...
int py_netsnmp_attr_set_string(PyObject *obj, char *attr_name, char *val,
        size_t len) {
    int ret = -1;
    if (obj && attr_name) {
        PyObject* val_obj = (
                val ? Py_BuildValue("s#", val, len) : Py_BuildValue(""));
...
int py_netsnmp_attr_get_oid(PyObject* obj, oid* p_oid, size_t maxlen,
        size_t* len) {
    PyObject* seq;
    int i, seqlen;
...
int __snprint_value(buf, buf_len, var, tp, type, flag)
    char * buf;size_t buf_len;netsnmp_variable_list * var;struct tree * tp;int type;int flag; {
    int len = 0;
    u_char* ip;
    struct enum_list *ep;
 ...
 */
void __py_netsnmp_update_session_errors(PyObject *session, char *err_str,
